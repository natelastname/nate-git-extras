"""Git-aware recursive copy.

This is an intentionally opinionated "cp" meant for project code/templates:

- Source must be a real directory (no files, no symlinks).
- Source must be inside a Git work tree.
- Copy is recursive by default.
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "src_dir",
        type=Path,
        help="Source directory (must be inside a git repo; symlinks are rejected)",
    )
    parser.add_argument(
        "dst",
        type=Path,
...
def main(argv: list[str] | None = None) -> None:
    ...
    git_cp(args.src_dir, args.dst, verbose=args.verbose, dry_run=args.dry_run)
